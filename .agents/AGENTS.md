...
### 5.3. Pipeline de Ofuscação Militar (`scripts/obfuscate.js`)
Antes da compilação do instalador (`npm run publish` ou `npm run dist`), o pipeline:
1. Faz backup de segurança do código limpo em `node_modules/.cache_src/`.
2. Aplica `javascript-obfuscator` em `main.js`, `preload.js`, `renderer.js` e `regis/encrypted_reg_data.js` com:
   * `compact: true`
   * `controlFlowFlattening: true` (probabilidade 1.0 — desestrutura completamente o fluxo linear do código)
   * `deadCodeInjection: true` (injeta instruções mortas para frustrar engenharia reversa)
   ...
   * `splitStrings: true` (divide strings em pedaços codificados em Base64/Hex)
   * `stringArrayEncoding: ['rc4']`
3. O `electron-builder` empacota os binários ofuscados.
4. O script restaura automaticamente o código limpo para o ambiente de desenvolvimento.
