  ...
  if (btnCheckUpdate) {
    btnCheckUpdate.addEventListener('click', async () => {
      ...
      try {
        ...
        if (res && res.dev) {
          if (cardStatusTitle) cardStatusTitle.textContent = 'Modo de Desenvolvimento';
          if (cardStatusDesc) cardStatusDesc.textContent = 'O auto-update funciona automaticamente no instalador empacotado (.exe).';
...
if (btnGameBooster) {
  btnGameBooster.addEventListener('click', async () => {
    ...
    btnGameBooster.textContent = 'Turbinando...';
    
    const res = await window.api.boostGameTurbo();
    if (res && res.success) {
      if (badgeGameBooster) {
        badgeGameBooster.style.display = 'inline';
        setTimeout(() => { badgeGameBooster.style.display = 'none'; }, 5000);
      }
      btnGameBooster.textContent = '✔ Booster Ativo!';
      alert('🔥 Modo Turbo Game Booster Ativado!\n\n• Plano de Energia Máxima ativado\n• Processo do emulador (HD-Player) elevado para Alta Prioridade\n• Memória RAM e lista de espera purgadas');
    } else {
      alert('Erro ao ativar Game Booster. Verifique os privilégios de Administrador.');
    }
