...
const presetSpeedBtns = document.querySelectorAll('.preset-speed-btn');

if (btnAutoBugFps) {
  btnAutoBugFps.addEventListener('click', async () => {
    btnAutoBugFps.disabled = true;
    const originalText = btnAutoBugFps.textContent;
    btnAutoBugFps.textContent = '⏳ Injetando Speedhack (500 ➔ 0.5)...';
    if (statusSpeedhackFps) {
      statusSpeedhackFps.style.display = 'block';
      statusSpeedhackFps.style.color = '#38bdf8';
      statusSpeedhackFps.textContent = 'Injetando no emulador aberto...';
    }

    try {
      const res = await window.api.bugFpsSpeedhack({ action: 'auto-bug' });
      if (res && res.success) {
        btnAutoBugFps.textContent = '🎉 FPS BUGADO COM SUCESSO!';
        btnAutoBugFps.style.background = 'linear-gradient(135deg, #10b981, #059669)';
        if (statusSpeedhackFps) {
          statusSpeedhackFps.style.color = '#10b981';
          statusSpeedhackFps.textContent = '✔ 240+ FPS Ativo! Limiter rompido (500 ➔ 0.5).';
        }
        setTimeout(() => {
          btnAutoBugFps.disabled = false;
          btnAutoBugFps.textContent = originalText;
          btnAutoBugFps.style.background = 'linear-gradient(135deg, #ef4444, #f59e0b)';
        }, 3000);
      } else {
        alert(res?.error || 'Erro ao aplicar o Speedhack. Verifique se o BlueStacks ou MSI está com o Free Fire aberto.');
        btnAutoBugFps.disabled = false;
        btnAutoBugFps.textContent = originalText;
        if (statusSpeedhackFps) {
          statusSpeedhackFps.style.color = '#ef4444';
          statusSpeedhackFps.textContent = '❌ ' + (res?.error || 'Emulador não encontrado.');
        }
      }
    } catch (e) {
      alert('Erro ao executar Speedhack: ' + e.message);
      btnAutoBugFps.disabled = false;
      btnAutoBugFps.textContent = originalText;
    }
  });
...
if (presetSpeedBtns) {
  presetSpeedBtns.forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      btn.disabled = true;
      try {
        const res = await window.api.bugFpsSpeedhack({ action: speedVal === 1.0 ? 'reset' : 'set-speed', speed: speedVal });
        if (res && res.success) {
          if (statusSpeedhackFps) {
            statusSpeedhackFps.style.display = 'block';
            statusSpeedhackFps.style.color = '#10b981';
            statusSpeedhackFps.textContent = `✔ Velocidade: ${speedVal}x aplicada!`;
          }
        } else {
          alert(res?.error || 'Erro ao alterar velocidade.');
        }
      } catch (e) {
        alert('Erro: ' + e.message);
      } finally {
