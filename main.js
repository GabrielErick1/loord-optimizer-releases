...
ipcMain.handle('boost-game-turbo', async () => {
  try {
    await runCmd('powercfg -duplicatescheme e9a42b02-d5df-448d-aa00-03f14749eb61').catch(() => {});
    await runCmd('powercfg -setactive 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c').catch(() => {});
    await runCmd('reg add "HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\HD-Player.exe\\PerfOptions" /v CpuPriorityClass /t REG_DWORD /d 3 /f /reg:64').catch(() => {});
    await runCmd('reg add "HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\HD-Player.exe\\PerfOptions" /v IoPriority /t REG_DWORD /d 3 /f /reg:64').catch(() => {});
    return { success: true };
  } catch (e) {
    return { success: false, error: e.message };
  }
